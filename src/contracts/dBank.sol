...
contract dBank {
  ...
  function balance(address add) public view returns (uint) {
    return etherBalanceOf[add];
  }
  ...
  function estimatedInterest(address add) public view returns(uint) {
    uint depositTime = block.timestamp - depositStart[add];
    uint interestPerSecond = 31668017 * (etherBalanceOf[add] / 1e16);
    uint interest = interestPerSecond * depositTime;  
    return interest;
  }
}